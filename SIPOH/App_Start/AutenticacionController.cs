...
public class AutenticacionController
{
    ...
    //{        
    //    intentosFallidos = ObtenerIntentosFallidos(usuario); // Función ficticia para obtener el número de intentos fallidos
    ...
    public static bool AutenticarUsuarioSeguridad(string usuario, string contrasena, out string mensajeError)
    {
        ...
        try
        {
            ...
            {
                connection.Open();
                // Verifica si la IP actual no está bloqueada
                string ipUsuario = HttpContext.Current.Request.UserHostAddress;

                if (IPBloqueadaUser(ipUsuario))
                {
                    mensajeError = "La IP está bloqueada";
                    return false;
                }

                // Verifica el número de intentos fallidos de inicio de sesión para la IP actual
                if (UsuarioExcedeIntentosFallidos(usuario))
                {
                    BloquearIP(ipUsuario, usuario);
                    mensajeError = "Demasiados intentos fallidos. La IP ha sido bloqueada";
                    return false;
                }

                RegistroAcceso(usuario, ipUsuario);
        ...
        {
            mensajeError = "Error en el sistema: " + ex.Message;
        }
        return false;
    }
